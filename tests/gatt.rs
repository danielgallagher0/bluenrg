...
fn add_service_16() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x02, 0xFD, 5, 0x01, 0x01, 0x02, 0x01, 3]));
}
...
fn include_service_16() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x03, 0xFD, 9, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x01, 0x07, 0x08]));
}
...
fn add_characteristic_pre_v72_16() {
    ...
    assert!(sink.wrote_header());
    assert!(sink
        .wrote(&[1, 0x04, 0xFD, 11, 0x01, 0x02, 0x01, 0x03, 0x04, 0x05, 0x13, 0x09, 0x07, 8, 1]));
}
...
fn delete_characteristic() {
    let mut sink = RecordingSink::new();
    {
        ...
        fixture
            .act(|controller| {
                controller
                    .delete_characteristic(ServiceHandle(0x0201), CharacteristicHandle(0x0403))
            })
            .unwrap();
    }
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x07, 0xFD, 4, 0x01, 0x02, 0x03, 0x04]));
}
...
fn delete_service() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x08, 0xFD, 2, 0x01, 0x02]));
}
...
fn delete_included_service() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x09, 0xFD, 4, 0x01, 0x02, 0x03, 0x04]));
}
...
fn set_event_mask() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x0A, 0xFD, 4, 0x09, 0x60, 0x00, 0x00]));
}
...
fn exchange_configuration() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x0B, 0xFD, 2, 0x01, 0x02]));
}
...
fn find_information_request() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x0C, 0xFD, 6, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06]));
}
...
fn read_by_type_request_16() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x0E, 0xFD, 9, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x01, 0x07, 0x08,]));
}
...
fn read_by_group_type_request_16() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x0F, 0xFD, 9, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x01, 0x07, 0x08,]));
}
...
fn execute_write_request() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x11, 0xFD, 3, 0x1, 0x2, 1]));
}
...
fn cancel_write_request() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x11, 0xFD, 3, 0x1, 0x2, 0]));
}
...
fn discover_all_primary_services() {
    let mut sink = RecordingSink::new();
    {
        ...
        fixture
            .act(|controller| {
                controller.discover_all_primary_services(hci::ConnectionHandle(0x0201))
            })
            .unwrap();
    }
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x12, 0xFD, 2, 0x1, 0x2]));
}
...
fn discovery_primary_services_by_uuid_16() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x13, 0xFD, 5, 0x01, 0x02, 0x01, 0x03, 0x04]));
}
...
fn find_included_services() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x14, 0xFD, 6, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06]));
}
...
fn discover_all_characteristics_of_service() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x15, 0xFD, 6, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06]));
}
...
fn discover_characteristics_by_uuid_16() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x16, 0xFD, 9, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x01, 0x07, 0x08]));
}
...
fn discover_all_characteristic_descriptors() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x17, 0xFD, 6, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06]));
}
...
fn read_characteristic_value() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x18, 0xFD, 4, 0x01, 0x02, 0x03, 0x04]));
}
...
fn read_characteristic_using_uuid_16() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x19, 0xFD, 9, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x1, 0x7, 0x8]));
}
...
fn read_long_characteristic_value() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x1A, 0xFD, 6, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6]));
}
...
fn read_multiple_characteristic_values() {
    let mut sink = RecordingSink::new();
    {
        ...
        fixture
            .act(|controller| {
                controller.read_multiple_characteristic_values(
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x1B, 0xFD, 11, 0x1, 0x2, 4, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xA]));
}
...
fn write_characteristic_value() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x1C, 0xFD, 9, 0x1, 0x2, 0x3, 0x4, 4, 1, 2, 3, 4]));
}
...
fn write_long_characteristic_value() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x1D, 0xFD, 11, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 4, 1, 2, 3, 4]));
}
...
fn write_characteristic_value_reliably() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x1E, 0xFD, 11, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 4, 1, 2, 3, 4]));
}
...
fn write_long_characteristic_descriptor() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x1F, 0xFD, 11, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 4, 1, 2, 3, 4]));
}
...
fn read_long_characteristic_descriptor() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x20, 0xFD, 6, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6]));
}
...
fn write_characteristic_descriptor() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x21, 0xFD, 9, 0x1, 0x2, 0x3, 0x4, 4, 1, 2, 3, 4]));
}
...
fn read_characteristic_descriptor() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x22, 0xFD, 4, 0x01, 0x02, 0x03, 0x04]));
}
...
fn write_without_response() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x23, 0xFD, 9, 0x1, 0x2, 0x3, 0x4, 4, 1, 2, 3, 4]));
}
...
fn signed_write_without_response() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x24, 0xFD, 9, 0x1, 0x2, 0x3, 0x4, 4, 1, 2, 3, 4]));
}
...
fn confirm_indication() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x25, 0xFD, 2, 0x1, 0x2]));
}
...
fn write_response() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x26, 0xFD, 13, 0x01, 0x02, 0x03, 0x04, 0, 0, 6, 1, 2, 3, 4, 5, 6,]));
}
...
fn set_security_permission() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x28, 0xFD, 5, 0x1, 0x2, 0x3, 0x4, 0x22]));
}
...
fn read_handle_value() {
    ...
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x2A, 0xFD, 2, 0x01, 0x02]));
}
...
fn read_handle_value_offset() {
    let mut sink = RecordingSink::new();
    {
        ...
        fixture
            .act(|controller| {
                controller.read_handle_value_offset(CharacteristicHandle(0x0201), 0x3)
            })
            .unwrap();
    }
    assert!(sink.wrote_header());
    assert!(sink.wrote(&[1, 0x2B, 0xFD, 3, 0x01, 0x02, 0x03]));
}
...
fn update_long_characteristic_value() {
    let mut sink = RecordingSink::new();
    {
        ...
        fixture
            .act(|controller| {
                controller.update_long_characteristic_value(
